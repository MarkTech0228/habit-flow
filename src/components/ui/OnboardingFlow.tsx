// src/components/ui/OnboardingFlow.tsx
// Moved from App.tsx lines 14020–14268
...
export const OnboardingFlow: React.FC<OnboardingFlowProps> = ({
  ...
  const accentBtn = isDark
    ? isGreen ? 'bg-green-500 hover:bg-green-400'
      : isLgbt ? 'bg-gradient-to-r from-red-500 to-blue-500 hover:opacity-90'
      : 'bg-pink-500 hover:bg-pink-400'
    : isGreen ? 'bg-green-600 hover:bg-green-700'
      : isLgbt ? 'bg-gradient-to-r from-red-600 to-blue-600 hover:opacity-90'
      : 'bg-pink-600 hover:bg-pink-700';

  const accentDot = isGreen ? 'bg-green-500' : isLgbt ? 'bg-indigo-500' : 'bg-pink-500';
      ...
      <div className={`w-full max-w-lg rounded-3xl shadow-2xl p-8 animate-pop ${
          ...
          {steps.map((_, idx) => (
            <div
              ...
              className={`h-1 flex-1 rounded-full transition-all duration-300 ${
                idx <= step ? accentDot : isDark ? 'bg-slate-700' : 'bg-slate-200'
              }`}
            />
          ))}
        </div>
        ...
        <div className="flex gap-3">
          ...
          <button
            ...
            className={`flex-1 px-6 py-4 rounded-2xl font-bold text-white transition shadow-lg ${accentBtn} ${
